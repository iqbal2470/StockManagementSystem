...
namespace StockManagementSystem.Forms.Products
{
    public partial class FrmProductNew : BaseForm
    {
        ...
        private readonly Font _bold9Font =
    new Font("Segoe UI", 9, FontStyle.Bold);

        private readonly Font _bold10Font =
    ...
    DataGridViewCellFormattingEventArgs e)
        {
            ...
            switch (columnName)
            {
                case "colStatus":
                    ...
                    bool active = row.CurrentStock > 0;
                    ...
                    e.CellStyle.Font = _bold9Font;

                    e.FormattingApplied = true;

                    break;

                case "colPurchasePrice":

                    if (e.Value != null)
                    {
                        e.Value =
                            ((decimal)e.Value).ToString("0.00");
                ...
                case "colCurrentStock":

                    int stock = row.CurrentStock;
        ...
        private async void FrmProductNew_Load(object sender, EventArgs e)
        {
            ...
            try
            {
                ...
                // Products aur Dashboard parallel
                await LoadProducts();
                await LoadDashboardCards();
                ...
                // Product Code aur Barcode parallel
                await GenerateProductCode();
    ...
    (await _productService.GetCurrentStockQuantityAsync()).ToString();
        ...
        private async Task LoadProducts()
        {
            ...
            try
            {
                ...
                var productList = products.Select(x => new ProductGridModel
                {
                    Id = x.Id,
                    ProductCode = x.ProductCode,
                    ProductName = x.ProductName,
        ...
        private Image? GetProductImage(string? imageName)
        {
            try
            {
                ...
                // Cache check
                if (_imageCache.TryGetValue(imageName, out Image? cachedImage))
                    return cachedImage;
